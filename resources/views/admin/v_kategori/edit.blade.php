...
@section('title', 'Edit Kategori - Admin SIMBO')

<main class="ml-[250px] w-[calc(100%-250px)] bg-dash-secondary min-h-screen font-worksans p-8 pt-24">
    
    <div class="mb-6 flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold font-montserrat text-neutral mb-1">Edit Kategori</h1>
            <p class="text-sm text-gray-500">Perbarui informasi nama kategori.</p>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-200 p-8 shadow-sm max-w-2xl">
        <form action="{{ route('admin.kategori.update', $kategori->id_kategori) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
