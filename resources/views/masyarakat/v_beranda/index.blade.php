...
@section('content')
    ...
    ══════════════════════════════════════════════════════════════ --}}
        <div x-show="!showPelajari" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
            x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200"
            x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="space-y-8">

            {{-- 1. Hero Section dengan Auto-Slideshow --}}
            <section x-data="{
                active: 0,
                images: [
                ...
                class="relative rounded-2xl p-8 border border-gray-200 shadow-sm overflow-hidden h-[300px] flex items-center">
                {{-- Container Gambar (Slideshow) --}}
                <template x-for="(img, index) in images" :key="index">
                    <div x-show="active === index" x-transition:enter="transition-opacity duration-1000 ease-in-out"
                        ...
                        class="absolute inset-0 bg-cover bg-center" :style="'background-image: url(' + img + ');'"></div>
                </template>
            ...
            <section class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div
                    class="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm flex flex-col items-center justify-center text-center border-t-4 border-t-neutral">
                ...
                <div
                    class="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm flex flex-col items-center justify-center text-center border-t-4 border-t-yellow-500">
                ...
                <div
                    class="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm flex flex-col items-center justify-center text-center border-t-4 border-t-green-500">
                ...
                <div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
                    @forelse ($aktivitasTerbaru as $item)
                        ...
                        <a href="{{ route('aktivitas.detail', ['id' => $item->id_laporan]) }}"
                            ...
                            <div
                                class="w-10 h-10 rounded-full {{ $config['bg'] }} {{ $config['text'] }} flex items-center justify-center shrink-0 mt-0.5">
                            ...
                            <div class="shrink-0 mt-0.5">
                                <span
                                    class="px-2.5 py-1 rounded-md {{ $config['badgeBg'] }} {{ $config['badgeText'] }} text-xs font-bold border {{ $config['badgeBorder'] }}">
                                    {{ $config['label'] }}
                                </span>
                            </div>
                        </a>
                        ...
                        @foreach ($beritaTerbaru as $berita)
                            <article
                                ...
                                <div class="p-5 flex-1 flex flex-col">
                                    ...
                                    <h4
                                        class="text-sm font-bold text-neutral mb-2 line-clamp-2 group-hover:text-primary transition-colors">
                                        {{ $berita->judul_berita }}
                                    </h4>
                                    <div
                                        class="mt-auto pt-4 flex items-center justify-between text-gray-400 border-t border-gray-50">
    ...
    ══════════════════════════════════════════════════════════════ --}}
        <div x-show="showPelajari" x-cloak x-transition:enter="transition ease-out duration-300"
            x-transition:enter-start="opacity-0 translate-y-3" x-transition:enter-end="opacity-100 translate-y-0"
            x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0"
            x-transition:leave-end="opacity-0 translate-y-3" class="space-y-10">
                ...
                @php
                    $steps = [
                        [
                            'icon' => 'ph-user-plus',
                            'title' => 'Daftar & Login',
                            'desc' => 'Buat akun gratis dan login ke platform Buitenzorg',
                        ],
                        [
                            'icon' => 'ph-camera',
                            'title' => 'Upload Foto',
                            'desc' => 'Upload foto yang ingin di laporkan',
                        ],
                        [
                            'icon' => 'ph-note-pencil',
                            'title' => 'Detail Laporan',
                            'desc' => 'Tulis deskripsi laporan terkait masalah yang terjadi',
                        ],
                        [
                            'icon' => 'ph-map-pin',
                            'title' => 'Atur Lokasi',
                            'desc' => 'Atur lokasi laporan sesuai tempat kejadian',
                        ],
                        [
                            'icon' => 'ph-magnifying-glass',
                            'title' => 'Tinjau Laporan',
                            'desc' => 'Pantau status laporan dan notifikasi laporan',
                        ],
                    ];
                @endphp
                    ...
                    @foreach ($steps as $i => $step)
                        <div class="flex flex-col items-center text-center group">
                            <div class="relative mb-5">
                                <div
                                    class="w-20 h-20 rounded-full bg-neutral text-white flex items-center justify-center shadow-md group-hover:bg-primary transition-colors duration-300">
                                    <i class="ph {{ $step['icon'] }} text-3xl"></i>
                                </div>
                                <span
                                    class="absolute -top-1 -right-1 w-6 h-6 rounded-full bg-primary text-white text-[11px] font-extrabold flex items-center justify-center border-2 border-white shadow-sm">
                                    {{ $i + 1 }}
                                </span>
                            </div>
                            <h4
                                class="text-sm font-extrabold text-neutral mb-2 group-hover:text-primary transition-colors">
                                {{ $step['title'] }}
                            </h4>
