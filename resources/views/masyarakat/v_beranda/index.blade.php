...
@section('content')
<div class="space-y-8 w-full" x-data="{ showPelajari: false }">
    ...
    <section class="rounded-2xl p-8 border border-gray-200 shadow-sm relative overflow-hidden bg-cover bg-center"
        ...
        <div class="relative z-10">
            ...
            <p class="text-sm text-gray-300 max-w-2xl leading-relaxed">
                Sampaikan aspirasi dan pengaduan Anda dengan mudah. Bersama, kita wujudkan pelayanan publik
                yang cepat, transparan, dan responsif untuk Kota Bogor.
            </p>
        </div>
    </section>
    ...
    <section class="flex flex-wrap gap-4 justify-center">
        ...
        {{-- Tombol Pelajari Lebih Lanjut — trigger Alpine modal --}}
    ...
    {{-- ══════════════════════════════════════════════════════════════
         MODAL — Pelajari Lebih Lanjut (Alpine.js x-show)
         Menutupi area konten utama (bukan seluruh layar)
         sesuai wireframe — sidebar & topbar tetap terlihat
    ══════════════════════════════════════════════════════════════ --}}
    <div x-show="showPelajari"
        ...
        style="left: var(--sidebar-width, 256px); top: var(--topbar-height, 80px);">

        <div class="max-w-4xl mx-auto px-8 py-12">

            {{-- Tombol Tutup --}}
            <button @click="showPelajari = false"
                class="absolute top-5 right-6 w-9 h-9 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition-colors">
                <i class="ph ph-x text-gray-600 text-lg"></i>
            </button>

            {{-- ── Section 1: Alur Pengaduan ──────────────────────── --}}
            <div class="text-center mb-14">
                <h2 class="text-3xl font-extrabold font-montserrat text-neutral mb-3">
                ...
                @foreach ($steps as $i => $step)
                    <div class="flex flex-col items-center text-center group">
                        ...
                        <div class="relative mb-4">
                            <div class="w-16 h-16 rounded-full bg-neutral text-white flex items-center justify-center shadow-md group-hover:bg-primary transition-colors duration-300">
                                <i class="ph {{ $step['icon'] }} text-2xl"></i>
                            </div>
                            <span class="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-primary text-white text-[10px] font-extrabold flex items-center justify-center border-2 border-white">
                                {{ $i + 1 }}
                            </span>
                        </div>
            ...
            {{-- ── Section 2: CTA ─────────────────────────────────── --}}
            <div class="bg-neutral rounded-2xl p-10 text-center text-white">

                <h2 class="text-3xl font-extrabold font-montserrat mb-4">
                    Siap Membuat Laporan?
                </h2>
                <p class="text-sm text-white/80 max-w-lg mx-auto leading-relaxed mb-8">
                ...
                <div class="flex items-center justify-center gap-3 flex-wrap">
                    <a href="{{ route('laporan.index') }}"
                        class="px-8 py-3.5 rounded-xl bg-white text-neutral font-extrabold text-sm hover:bg-gray-100 transition-all shadow-md hover:shadow-lg">
